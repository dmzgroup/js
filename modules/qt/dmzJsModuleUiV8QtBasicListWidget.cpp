...
dmz::V8Value
dmz::JsModuleUiV8QtBasic::_list_widget_item_fg_brush (const v8::Arguments &Args) {
   ...
   if (self) {
      ...
      if (item) {
         ...
         if (Args.Length ()) {
            ...
            QBrush *brush = self->_to_gbrush (Args[0]);
            if (brush) { item->setForeground (*brush); }
         }

         result = self->create_v8_gbrush (new QBrush (item->foreground ()));
      }
   }
...
dmz::JsModuleUiV8QtBasic::_init_list_widget_item () {
   ...
   proto->Set ("foreground", v8::FunctionTemplate::New (_list_widget_item_fg_brush, _self));
