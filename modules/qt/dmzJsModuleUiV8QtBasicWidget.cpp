...
dmz::V8Value
dmz::JsModuleUiV8QtBasic::_widget_raise (const v8::Arguments &Args) {
...
dmz::V8Value
dmz::JsModuleUiV8QtBasic::_widget_update_geometry (const v8::Arguments &Args) {
...
dmz::V8Value
dmz::JsModuleUiV8QtBasic::_widget_move (const v8::Arguments &Args) {
   ...
   if (self) {
      ...
      if (widget) {
         ...
         if (Args.Length () == 2) {
            ...
            widget->move (v8_to_uint32 (Args[0]), v8_to_uint32 (Args[1]));
         }
      }
   }
...
dmz::V8Value
dmz::JsModuleUiV8QtBasic::_widget_geometry (const v8::Arguments &Args) {
   ...
   if (self) {
      ...
      if (widget) {
         ...
         if (Args.Length ()) { widget->setGeometry (v8_to_qrect (Args[0])); }
         result = qrect_to_v8 (widget->geometry ());
      }
   }
